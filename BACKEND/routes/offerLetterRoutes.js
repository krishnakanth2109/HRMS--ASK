...
router.post("/send-email", protect, onlyAdmin, async (req, res) => {
  try {
    ...
    console.log("📧 Sending full offer email:", { employeeId, companyName, hasPdf: !!pdfBase64 });
    ...
    // Generate token for offer response
    ...
    if (pdfBase64) {
      try {
        ...
        console.error("❌ Error preparing PDF attachment:", pdfErr.message);
    ...
    const emailHtml = `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #e2e8f0;">
        ...
        <div style="padding: 0 40px 40px; text-align: center;">
          ...
          <div>
            <a href="${acceptUrl}" style="display: inline-block; background: linear-gradient(135deg, #10b981, #059669); color: white; padding: 14px 40px; text-decoration: none; border-radius: 8px; font-weight: 700; font-size: 15px; margin-right: 12px; margin-bottom: 10px;">
              ✅ I Accept the Offer
            </a>
        ...
        <div style="background: #f8fafc; padding: 24px 40px; border-top: 1px solid #e2e8f0; text-align: center;">
          <p style="color: #94a3b8; font-size: 12px; margin: 0;">
            This offer expires on ${emp.expires_at.toLocaleDateString()}
          </p>
    ...
    const mailOptions = {
      ...
      to: emp.email,
      subject: `Offer Letter - ${emp.name} - ${company}`,
      html: emailHtml,
    };
...
router.get("/respond", async (req, res) => {
  try {
    ...
    const employee = await OfferLetterEmployee.findOne({ offer_token: token });
      ...
      return res.status(404).send(`
        <html>
          <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
            ...
            <p>The offer link you're trying to access is invalid or has expired.</p>
    ...
    if (employee.expires_at < new Date()) {
      return res.status(400).send(`
        <html>
          <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
            <h1 style="color: red;">Offer Expired</h1>
            <p>This offer has expired on ${employee.expires_at.toLocaleDateString()}.</p>
            <p>Please contact HR for a new offer letter.</p>
          </body>
        </html>
      `);
    }
        ...
        <html>
          <head>
            <style>
              ...
              .container { background: white; border-radius: 20px; padding: 40px; max-width: 500px; margin: 0 auto; box-shadow: 0 10px 40px rgba(0,0,0,0.1); }
          ...
          <body>
            <div class="container">
              ...
              <p>Thank you for accepting the offer from <strong>${employee.companyName || 'our company'}</strong>.</p>
              <p>You will receive onboarding details and further instructions within 2-3 business days.</p>
        ...
        <html>
          <head>
            <style>
              ...
              p { color: #4a5568; line-height: 1.6; margin-bottom: 20px; }
              .btn { display: inline-block; background: #6b7280; color: white; padding: 12px 30px; text-decoration: none; border-radius: 8px; margin-top: 20px; }
            </style>
          </head>
          <body>
            <div class="container">
              <h1>❌ Offer Declined</h1>
              <p>We acknowledge your decision to decline the offer from <strong>${employee.companyName || 'our company'}</strong>.</p>
